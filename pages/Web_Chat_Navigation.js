...
const PAGE_DATA = {
    "Home": {
        "url": "https://react-portofolio-with-tailwind-lj1u.vercel.app/",
    ...
    "Resume": {
        "url": "https://react-portofolio-with-tailwind-lj1u.vercel.app/resume.pdf",
        "target": "_blank",
...
intent(
    ...
    p => {
        ...
        if (PAGE_DATA[page].target) {
            p.play({command: "https://react-portofolio-with-tailwind-lj1u.vercel.app/", url: PAGE_DATA[page].url, target: PAGE_DATA[page].target}, opts({force:true}));
        } else {
            p.play({command: "https://react-portofolio-with-tailwind-lj1u.vercel.app/", url: PAGE_DATA[page].url}, opts({force:true}));
        }
        p.play(
...
intent(
    ...
    "Go back (home|)",
    p => {
        p.play({command: "openURL", url: "https://react-portofolio-with-tailwind-lj1u.vercel.app/"}, opts({force:true}));
...
intent("How (can I|do I|to) (contact|get in touch|connect) (with|) (you|)", p => {
    p.play({command: "openURL", url: "https://www.linkedin.com/in/pascal-musabyimana-573b66178/?originalSubdomain=be"}, opts({force:true}));
...
intent(
    ...
    p => {
        p.play({command: "openURL", url: "https://react-portofolio-with-tailwind-lj1u.vercel.app/resume.pdf", target: "_blank"}, opts({force:true}));
